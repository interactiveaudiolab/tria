...
from .mask import cosine_schedule
from .mask import format_seed
from .sample import mask_by_confidence
...
class TRIA(torch.nn.Module):
    def __init__(
        ...
        )  # No bias on head, to allow weight-sharing
        if share_emb:
    ...
    def forward(
        ...
        feats_mask : torch.Tensor
            Boolean tensor indicating unmasked feature positions (True where
            unmasked, False where masked); shape (n_batch, n_frames')
        lengths : torch.Tensor
            Optional valid lengths of token sequences, to prevent attention to
            padded positions; shape (n_batch,)
        """
        ...
        if lengths is not None:
            lengths = lengths.clamp_max(n_frames).long()
        else:
            lengths = torch.full((n_batch,), n_frames, dtype=torch.long, device=device)
        ...
        # Embed tokens
        codebook_offsets = (
            torch.arange(n_codebooks, dtype=tokens.dtype, device=device).reshape(
                1, -1, 1
            )
            * self.codebook_size
        )  # (1, n_codebooks, 1)
        tokens = tokens + codebook_offsets  # (n_batch, n_codebooks, n_frames)
        tokens_emb = self.tokens_emb(
            tokens
        )  # (n_batch, n_codebooks, n_frames, n_channels)
        ...
        mask_pos = (
            torch.arange(n_codebooks, dtype=tokens.dtype, device=device)[None, :]
            == codebook[:, None]
        )  # (n_batch, n_codebooks)
        mask_pos = torch.logical_and(
            mask_pos.unsqueeze(-1), ~tokens_mask
        )  # (n_batch, n_codebooks, n_frames)

        tokens_emb = tokens_emb + (
            mask_pos.unsqueeze(-1).float()
        ) * self.tokens_mask_emb.reshape(
            1, 1, 1, -1
        )  # (n_batch, n_codebooks, n_frames, n_channels)
        ...
        if self.training and self.p_token_dropout > 0.0:
            ...
            below = (
                torch.arange(n_codebooks, device=device)[None, :, None]
                < codebook[:, None, None]
            )  # (n_batch, n_codebooks, 1)
            eligible = below & feats_mask.bool()  # (n_batch, n_codebooks, n_frames)
            drop = (
                torch.rand(n_batch, 1, n_frames, 1, device=device)
                < self.p_token_dropout
            ) & eligible[..., None]
    ...
    def inference(
        ...
        if lengths is not None:
            lengths = lengths.to(device).clamp_max(n_frames).long()
        else:
            lengths = torch.full(
                (n_batch,),
                n_frames,
        ...
        valid = (
            torch.arange(n_frames, device=device)[None, :] < lengths[:, None]
        )  # (n_batch, n_frames)
        ...
        feats_mask = (
            torch.nn.functional.interpolate(
                feats_mask.unsqueeze(1).float().to(device),
                n_frames,
                mode="nearest",
            )
            .squeeze(1)
        ...
        # Mark invalid token sequence positions as "already unmasked" to exclude from inference
        tokens_mask = torch.logical_or(tokens_mask, ~valid[:, None, :])
        ...
        state = [
            format_seed([s + 10007 * cb for s in seed]) for cb in range(n_codebooks)
        ]
        ...
        for codebook_idx, (
            ...
            _causal_bias = _causal_bias or 0.0
            assert 0.0 <= _causal_bias

            _temp = _temp or 1.0
            assert 0.0 < _temp

            _mask_temp = _mask_temp or 0.0
            assert 0.0 <= _mask_temp
            ...
            for _iter in range(_iterations):
                ...
                if _guidance_scale:
                    ...
                    feats_mask_cfg = torch.cat(
                        [feats_mask, torch.zeros_like(feats_mask)], dim=0
                    )

                    lengths_cfg = torch.cat([lengths, lengths], dim=0)
                    ...
                    logits_cond, logits_uncond = self.forward(
                        ...
                        2, dim=0
                    )  # (n_batch, n_codebooks, n_frames, codebook_size) x2

                    logits = logits_uncond + _guidance_scale * (
                ...
                tgt_p_mask = cosine_schedule(
                    torch.tensor([t] * n_batch, device=device)
                )  # (n_batch,)

                # Compute target and actual number of masked positions in current
                # codebook
                tgt_n_masked = torch.floor(
                    tgt_p_mask * n_masked_init[:, codebook_idx]
                ).long()  # (n_batch,)
